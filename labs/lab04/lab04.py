def divide(quotients, divisors):
    ...
    return {
        quotients[i]: [
            divisors[j] for j in range(len(divisors)) if divisors[j] % quotients[i] == 0
        ]
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_example = make_city('city_example', lat, lon)
    if distance(city_example, city_a) < distance(city_example, city_b):
