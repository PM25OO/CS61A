            ...
            while not (place.is_hive):
                if i >= self.lower_bound and i <= self.upper_bound:
                    if (place.bees):
                        ...
                        return nearest(place.entrance,i + 1)
                else:
                    return nearest(place.entrance,i + 1)
            return None
        return nearest(self.place, 0)
        # END Problem 3 and 4
    ...
    def action(self, gamestate):
        if self.cooldown == 0:
            if(self.place.bees):
                ...
                Insect.reduce_health(target_bee, target_bee.health)
                self.cooldown += 1
        if self.cooldown > 0:
            if not self.cooldown > self.chew_cooldown:
                self.cooldown += 1
            else:
                self.cooldown = 0
# END Problem 7
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while (place != None):
            if place.ant and place.ant.is_buffed == False :
                place.ant.double()
            if place.ant and place.ant.is_container and place.ant.ant_contained and place.ant.ant_contained.is_buffed == False :
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        if target is not None:
            target.action = make_slow(target.action, target, 5)
        # END Problem EC 1
def make_slow(action, bee, duration):
    """Return a new action method that slows the bee for DURATION turns."""
    def slowed_action(gamestate):
        if duration > 0:
            if gamestate.time % 2 == 0:
                action(gamestate)
            # Odd time: skip action
            slowed_action.duration -= 1
        else:
            action(gamestate)
    slowed_action.duration = duration
