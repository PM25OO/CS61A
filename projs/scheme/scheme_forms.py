...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        env.define(str(signature), scheme_eval(expressions.rest.first, env))
        return signature
        # END PROBLEM 4
        ...
        "*** YOUR CODE HERE ***"
        sig = signature.first
        schemeFunction = do_lambda_form(Pair(signature.rest, expressions.rest), env)
        env.define(sig, schemeFunction)
        return sig
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    currentExpr = scheme_eval(expressions.first, env)
    restExpr = expressions.rest
...
def do_or_form(expressions, env):
    ...
    currentExpr = scheme_eval(expressions.first, env)
    restExpr = expressions.rest
    if is_scheme_false(currentExpr):
        return do_or_form(restExpr, env)
    return currentExpr
    # END PROBLEM 12
